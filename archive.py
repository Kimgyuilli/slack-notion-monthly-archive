...
def parse_args(
    ...
    parser.add_argument(
        "--mock",
        action="store_true",
        help="Slack 없이 샘플 데이터로 미리보기",
    )

    parser.add_argument(
        "--publish",
        action="store_true",
        help="Notion에 실제 페이지 생성",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "이미지 하나의 최대 다운로드 "
            f"크기(MB). 기본값 {DEFAULT_MAX_IMAGE_MB}"
        ),
    )
...
def run(
    ...
    if args.mock:
        ...
        workspace_url = (
                workspace_url
                or "https://demo-workspace.slack.com"
        )
        ...
        auto_join_public = environment_flag(
    ...
    if (
            not notion_token
            or not notion_data_source
    ):
        raise ArchiveError(
            "게시하려면 NOTION_TOKEN과 "
            "NOTION_DATA_SOURCE_ID가 필요합니다."
        )
    ...
    for channel in channels:
        ...
        if existing:
            ...
            print(
                (
                    f"skipped  {title}  "
                    f"{existing.get('url') or existing['id']}"
                )
            )

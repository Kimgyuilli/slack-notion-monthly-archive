...
- 상태가 `완료`인 행만 "이미 처리됨"으로 건너뜁니다.
- `진행 중`이나 `실패`로 남은 행은 본문이 불완전할 수 있어 `미완료`로 보고하고 경고를 출력하며, 종료 코드 `1`로 끝냅니다. 중복 판정이 `채널 + 기간`만 보기 때문에, 이런 행을 그대로 두면 재실행해도 영원히 건너뛰게 됩니다.
- 미완료 행을 다시 처리하려면 해당 행을 휴지통으로 옮긴 뒤 재실행합니다.
